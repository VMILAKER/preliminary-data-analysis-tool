...
def compute_dqi(df, iqr_outliers_df=None):
    ...
    completeness = (1 - total_nan / total_cells) * 100 if total_cells > 0 else 100
    ...
    for col in df.columns:
        ...
        if nu <= 1:
            ...
            if nu == len(df[col].dropna()) and nu > 50:
                bad_card += 1
    card_score = ((total_cols - bad_card) / total_cols) * 100 if total_cols > 0 else 100
...
def detect_mixed_types(df, sample_size=2000, min_ratio=0.05):
    ...
    for col in df.select_dtypes(include=["object"]).columns:
        ...
        if len(series) < 10:
            continue
        sample = series if len(series) <= sample_size else series.sample(sample_size, random_state=42)
...
def detect_date_columns(df, threshold=0.3):
    ...
    for col in df.select_dtypes(include=["object"]).columns:
        ...
        if ratio >= threshold:
            fmt_str = ", ".join(sorted(formats_found)[:3]) if formats_found else "разные"
            results.append({
                "Колонка": col,
                "Тип данных": str(df[col].dtype),
